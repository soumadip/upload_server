...
def view_report(filename):
    ...
    if record:
        ...
        # 1. Compiler errors get the full log so they can debug syntax
        if db_status in ['Compilation Error', 'Syntax Error']:
            return f"Status: {db_status}\n\n{db_log}"

        # 2. Parse the secure test-case breakdown from the execution log
        safe_summary = []
        for line in db_log.split('\n'):
            # Grab only the header lines generated by the grader (e.g., "--- Test 2 Failed ---")
            if line.startswith('--- Test') and any(x in line for x in ['Passed', 'Failed', 'Error']):
                safe_summary.append("• " + line.replace('---', '').strip())

        summary_text = "\n".join(safe_summary) if safe_summary else "No test case data found."

        # 3. Return the breakdown without the inputs/outputs
        return (f"Final Evaluation Status: {db_status}\n\n"
                f"Compilation successful.\n\n"
                f"Test Case Breakdown:\n{summary_text}\n\n"
                f"(Detailed test inputs and expected outputs are hidden to maintain academic integrity.)")
